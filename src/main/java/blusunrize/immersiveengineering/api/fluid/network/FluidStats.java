...
/**
 * Throughput bookkeeping for one main, in millibuckets.
 * <p>
 * Steady-state cost is two int adds per tick; the per-second history is a pair of fixed
 * primitive ring buffers, so nothing is allocated while the server runs. Only the lifetime
 * totals are persisted -- the history is a live view, not save data.
 * <p>
 * The deliberate mirror of {@code GridStats}. Kept separate rather than shared because the two
 * networks are separate on purpose (see {@link FluidNetConfig}), and because a shared stats
 * object would be the first thread to pull the two engines back together.
 *
...
public class FluidStats
{
	...
	/**
	 * Starts a fresh tick. Rolls the previous tick's counters into {@code lastTick*} so a GUI
	 * reading mid-tick never sees a half-accumulated value.
	 */
	...
	/**
	 * Closes the tick, folding it into the current one-second sample and rolling the ring buffer
	...
	/**
	 * @return the per-second intake history, oldest sample first, length {@link #getSampleCount()}
	 */
	...
	private int[] copyOrdered(int[] ring)
	{
		...
		//The ring is full once samplesRecorded saturates, at which point head is also the oldest
		//slot. Before that, samples start at 0 and head is one past the newest.
	...
	/**
	 * Drops the live history and per-tick counters but keeps the lifetime totals, which are a
	 * meter reading rather than a measurement.
	 */
	...
	/**
	 * @param live also include the running counters and the history ring. Set when writing for a
	 *             GUI sync and clear when writing save data -- the history is a live view of the
	 *             last minute, and persisting it would only reload a stale graph that the next
	 *             second overwrites anyway.
	 */
